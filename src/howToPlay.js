...
export class HowToPlayRenderer {
    ...
    renderReconstructDemo() {
        ...
        const layout = new Layout(size, origin);
        ...
        const drawList = hexes.map((h, i) => {
            const hex = new Hex(h.q, h.r, h.val, 1, 'main');
        ...
        drawList.forEach(item => {
            const hex = new Hex(item.data.q, item.data.r, item.data.val, 1, 'main');
        ...
        if (localT >= moveStart && localT < moveEnd) {
            const leftHex = new Hex(hexes[leftIdx].q, hexes[leftIdx].r, valLeft, 1, 'main');
            const rightHex = new Hex(hexes[rightIdx].q, hexes[rightIdx].r, valRight, 1, 'main');
    ...
    renderInjectDemo() {
        ...
        hexes.forEach(h => {
            const center = layout.hexToPixel(new Hex(h.q, h.r, 0, 0));
            h.y = center.y;
        });
        ...
        hexes.forEach(hData => {
            ...
            const hex = new Hex(hData.q, hData.r, currentH, owner, 'main');
        ...
        hexes.forEach(hData => {
            ...
            if (t >= myStartTime && t < myImpactTime) {
                ...
                const hex = new Hex(hData.q, hData.r, 0, 0);
    ...
    renderOverloadDemo() {
        ...
        const owner = (h === 0) ? 0 : 1;
        const hex = new Hex(0, 0, h, owner, 'main');
        hex.visualHeight = h;
    ...
    renderObjectiveCore() {
        ...
        const tempLayout = new Layout(size, origin);

        const coreHex = new Hex(0, 0, 3, 1, 'main');
