...
- **`initialState` / `onSaveState(state)`**:
  共通ライブラリが管理する進行状態を保存・復元します。アプリ側は state の内部構造を解釈せず、そのまま保存・復元します。
- **`nextButtonSelector`**:
  チュートリアルの Next / OK ボタン制御を共通ライブラリへ委譲します。指定時、アプリ側は `advanceScenario()` を直接呼びません。
