...
- **アニメーション**:
  - **落下エフェクト / リプルエフェクト**: 物理演算に基づいた動的描画。
  - **エネルギーバースト (先攻決定演出)**: 以下の4フェーズで構成される。
    - `gathering`: 画面外から中心へ粒子が集束。
    - `fusion`: 中心でのエネルギーの脈動と膨張。
    - `burst`: ターゲット（プレイヤーラベル）への飛散と高速誘導。
    - `stabilized`: 着弾時の色彩・発光遷移（Player -> Energy(琥珀色) -> Player）による安定化。
  - **再構築ドット**: 放物線を描くパーティクルによるエネルギー分配。
